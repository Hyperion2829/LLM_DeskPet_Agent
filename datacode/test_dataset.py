...
print("📦 正在加载裂变种子库...")
try:
    with open(os.path.join(BASE_DIR, "mega_seed_inputs.json"), "r", encoding="utf-8") as f:
    ...
    print("❌ 找不到 mega_seed_inputs.json 或 mega_view_inputs.json，请确保已运行裂变脚本！")
    exit(1)
...
def generate_data(mode, seed_tuple=None):
    ...
    metaphor_rule = "【允许比喻】本次回复你可以适度使用一句关于农耕或因果的比喻。" if allow_metaphor else "【严禁比喻！非常重要】本次回复请使用绝对符合黍人设的日常语言！绝对不允许出现“庄稼、种子、收成、风雨、因果”等任何比喻！"

    if mode == "text":
        user_input, action = seed_tuple
        action_req = f"必须使用 action: {action}" if action else "根据情境自己选择合适的 action"
        prompt = f"""
        用户说："{user_input}"
        请直接输出你作为“黍”的回复。注意由于这是【测试集生成】，请务必给出与之前不同的全新视角的回答。
        【动态约束】：{metaphor_rule}
        【动作约束】：{action_req}
        """
    elif mode == "jailbreak":
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.95 # 针对测试集略微提高温度，以增加与训练集的差异
        )
...
# ==================== 主控流程 ====================
TARGET_SIZE = 1000  # ✨ 生成 1000 条测试数据
text_ratio = 0.70
view_ratio = 0.20
jailbreak_ratio = 0.10

target_text = int(TARGET_SIZE * text_ratio)
target_view = int(TARGET_SIZE * view_ratio)
target_jailbreak = TARGET_SIZE - target_text - target_view

data = []

print("🧪 开始生成 1000 条 测试集 (Test Set) 数据...\n")

# ================= 1. 生成 Text 数据 =================
text_pool = seed_inputs.copy()
random.shuffle(text_pool)

with tqdm(total=target_text, desc="🚀 测试集 - 正常对话", unit="条") as pbar:
    while len(data) < target_text:
        if not text_pool:
            text_pool = seed_inputs.copy()
            random.shuffle(text_pool)
...
with tqdm(total=target_view, desc="👀 测试集 - 视觉感知", unit="条") as pbar:
    while len(data) - current_len < target_view:
        if not v_pool:
            v_pool = view_pool.copy()
            random.shuffle(v_pool)
...
with tqdm(total=target_jailbreak, desc="🛡️ 测试集 - 抗越狱", unit="条") as pbar:
    while len(data) - current_len < target_jailbreak:
        if not j_pool:
            j_pool = jailbreak_pool.copy()
            random.shuffle(j_pool)
...
# ==================== 保存数据 ====================
save_path = os.path.join(BASE_DIR, "test_sft.json")
